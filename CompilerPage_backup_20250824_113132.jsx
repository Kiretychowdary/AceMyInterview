...
import { validateJudge0Setup, getJudge0Headers, getJudge0Urls, testJudge0Connection, testMultipleEndpoints, getDirectJudge0Headers, getDirectJudge0Urls } from '../utils/judge0Config';
...
function CompilerPage() {
  ...
  // Enhanced code execution with Judge0 - with rate limiting support and fallback
  const executeCodeWithJudge0 = async (sourceCode, languageId, input = "", timeout = 5) => {
    ...
    // Try RapidAPI first, then direct Judge0 as fallback
    const endpoints = [
      {
      ...
      },
      {
        ...
        headers: getDirectJudge0Headers(),
        urls: getDirectJudge0Urls()
      }
    ];
      ...
      try {
        ...
        const submitWithRetry = async (payload, maxRetries = 3) => {
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              console.log(`Judge0 submission attempt ${attempt}/${maxRetries} using ${endpoint.name}...`);
              const response = await axios.post(
                endpoint.urls.submit,
                payload,
                {
              ...
              if (error.response?.status === 429) {
                ...
                if (attempt < maxRetries) {
                  ...
                  throw new Error(`Rate limit exceeded. Please wait a few minutes before running code again. 
...
💡 Solutions:
1. Wait 1-2 minutes before running code
2. Upgrade to Judge0 paid plan for higher limits
3. Use fewer test runs per minute`);
                }
              }
              throw error;
            }
        ...
        try {
          ...
          console.log(`Trying ${endpoint.name} submission with plain text...`);
          submitResponse = await submitWithRetry(plainPayload);
        } catch (plainError) {
          if (plainError.message.includes('Rate limit exceeded')) {
            throw plainError; // Re-throw rate limit errors
          }
          
          console.log(`Plain text failed for ${endpoint.name}, trying base64...`, plainError.message);
        ...
        console.log(`${endpoint.name} submission successful, token:`, token);
        ...
        // Poll for result with rate limiting protection
        let attempts = 0;
        const maxAttempts = 20;
        const pollInterval = 2000; // Increased to 2 seconds to reduce API calls
        ...
        const pollResult = async () => {
          ...
          try {
            const resultResponse = await axios.get(
              endpoint.urls.getResult(token),
              { headers: endpoint.headers, timeout: 8000 }
            );
            ...
            // Execution completed - process results
            return result;

          } catch (pollError) {
            if (pollError.response?.status === 429) {
              console.log('Rate limited while polling. Waiting longer...');
              // Increase wait time for rate limited polling
              const waitTime = Math.min(pollInterval * 2, 5000); // Max 5 seconds
              await new Promise(resolve => setTimeout(resolve, waitTime));
              return pollResult(); // Retry with longer wait
            }
            throw pollError;
          }
        };

        // Start polling and get the final result
        ...
        console.log(`✅ Successfully executed code using ${endpoint.name}`);
        return {
        ...
        // If it's a 403 error with RapidAPI, try the next endpoint
        if (error.response?.status === 403 && endpoint.name.includes('RapidAPI')) {
          console.log('🔄 403 error with RapidAPI, trying direct Judge0...');
          continue;
        }
        
        // If it's any other error, try the next endpoint
        console.log(`🔄 Error with ${endpoint.name}, trying next endpoint...`);
        continue;
      }
    }
    ...
    if (lastError?.response?.status === 403) {
      throw new Error(`Authentication failed: Your RapidAPI key may not have Judge0 subscription. 

🔑 Solutions:
1. Check your RapidAPI subscription includes Judge0 CE
2. Verify your API key is correct: ${lastError.message}
3. Try using the direct Judge0 endpoint (free tier)

💡 Alternative: Use direct Judge0 at https://ce.judge0.com (no API key needed)`);
    }
    
    throw new Error(`All execution endpoints failed. Last error: ${lastError?.message || 'Unknown error'}`);

    try {
            console.log(`Judge0 submission attempt ${attempt}/${maxRetries}...`);
            const response = await axios.post(
              judge0Urls.submit,
              payload,
              {
